...
class Messenger(object):
    ...
    async def send_synchronize_data_transaction(self,private_key,data):
        transaction_signer = self._crypto_factory.new_signer(
            secp256k1.Secp256k1PrivateKey.from_hex(private_key))

        batch = make_synchronize_data_transaction(
    ...
    async def send_create_user_transaction(self, private_key, username, email, timestamp):
        transaction_signer = self._crypto_factory.new_signer(
            secp256k1.Secp256k1PrivateKey.from_hex(private_key)
        )
